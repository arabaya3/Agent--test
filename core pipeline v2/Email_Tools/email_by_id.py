...
def search_email_by_id(email_id: str) -> str:
    ...
    result = []
    result.append(f"=== Email Details for ID: '{email_id}' ===\n")
    ...
    try:
        ...
        api_url = f"https://graph.microsoft.com/v1.0/users/{user_email}/messages/{email_id}"
        params = {
            '$select': 'id,subject,from,receivedDateTime,bodyPreview,body,hasAttachments,isRead,toRecipients,ccRecipients,importance,flag'
        }
        headers = {
        ...
        if flag_info:
            result.append(f"Flagged: {flag_info.get('flagStatus', 'notFlagged')}")

        body_content = email.get('body', {})
        if body_content:
            result.append(f"\nBody ({body_content.get('contentType', 'text')}):")
            result.append("-" * 40)
            result.append(body_content.get('content', 'No content'))
