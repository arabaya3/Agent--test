...
def search_meetings_by_title(title, headers, user_id=None):
    ...
    url = f"https://graph.microsoft.com/v1.0/users/{target_user}/events?$filter=contains(subject,'{title}')&$orderby=start/dateTime desc&$top=100"
    all_meetings = []
    try:
        while url:
...
def main():
    ...
    if not target_user:
        print("Error: DEFAULT_USER_ID not set. Cannot search meetings.")
        return
    ...
    retrieve_meetings_by_title(title, target_user)
