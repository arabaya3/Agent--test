...
def search_emails_by_sender_and_date_range(sender, start_date, end_date, headers, email_ids, user_id=None):
    ...
    # Use the same approach as by_date_range.py - direct query with date filter
    base_url = "https://graph.microsoft.com/v1.0"
    url = (
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for email in batch:
                email_sender = email.get("from", {}).get("emailAddress", {}).get("address", "")
                email_sender_name = email.get("from", {}).get("emailAddress", {}).get("name", "")
                
                # Check if sender matches in email address or display name
                if (sender.lower() in email_sender.lower() or 
                    sender.lower() in email_sender_name.lower()):
...
def main():
    ...
    # Use the date range function and display results
    emails = search_emails_by_sender_and_date_range(sender, start_date, end_date, headers, email_ids, target_user)
